...
* /
    ...
    void DCSClientProtob::send(RaftGroupMsg const& message, handle_resp complete) {
    ...
    inline std::shared_ptr< nuraft::resp_msg > toResponse(RaftMessage const& raft_msg) {
        ...
        auto message = std::make_shared< grpc_resp >(base.term(), (nuraft::msg_type)base.type(), base.src(),
                                                     base.dest(), resp.next_index(), resp.accepted());
    ...
    void GroupClientProtob::send(std::shared_ptr< nuraft::req_msg > & req, nuraft::rpc_handler & complete, uint64_t)
        override {
        assert(req && complete);
    ...
    void GroupClientProtob::send(RaftMessage const& message, handle_resp complete) {
